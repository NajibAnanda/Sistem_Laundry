...
def cek_layanan_duplikat(nama, harga, id_untuk_diabaikan=None):
    """
    Mengecek apakah layanan dengan nama dan harga yg sama sudah ada.
    Jika 'id_untuk_diabaikan' diberikan, itu artinya kita sedang mode UPDATE,
    jadi kita abaikan data lama dari layanan itu sendiri.
    """
    global _database_layanan
    for layanan in _database_layanan:
        # Lewati item yg sedang kita edit
        if layanan["id_layanan"] == id_untuk_diabaikan:
            continue
        
        # Cek duplikat (case insensitive untuk nama)
        if layanan["nama_layanan"].lower() == nama.lower() and layanan["harga_per_kg"] == harga:
            return True # Ditemukan duplikat
    return False # Aman, tidak ada duplikat
